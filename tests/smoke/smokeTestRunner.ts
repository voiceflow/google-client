    ...
    await Promise.each(requests, async ({ request, response }) => {
      try {
        ...
        properties.forEach((prop) => {
          ...
          if (propPath === 'payload.google.userStorage') {
            // cant compare forceUpdateToken, it's randomly generated. expected to be a string though
            actual = JSON.parse(actual);
            expect(typeof actual.data.forceUpdateToken).to.eql('string');
            expect(actual.data.forceUpdateToken.length > 0).to.eql(true);

            delete actual.data.forceUpdateToken;
            expected = JSON.parse(expected);
            delete expected.data.forceUpdateToken;
          }
