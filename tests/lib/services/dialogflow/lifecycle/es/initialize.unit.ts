...
import InitializeManager from '@/lib/services/dialogflow/lifecycle/es/initialize';
...
describe('initializeManager unit tests', async () => {
  describe('build', () => {
    ...
    it('first session', async () => {
      ...
      expect(runtime.storage.set.args[1]).to.eql([S.LOCALE, req.queryResult.languageCode]);
      expect(runtime.storage.set.args[2]).to.eql([S.USER, req.session]);
    ...
    it('second session', async () => {
      ...
      const { services, runtime, req } = generateFakes();
    ...
    describe('restart logic', () => {
      describe('shouldRestart', () => {
        it('stack empty', async () => {
          const { services, runtime, req, metaObj } = generateFakes();
      ...
      describe('resume prompt', () => {
        it('resume stack 0', async () => {
          const { services, runtime, req, metaObj, topStorage, resumeFrame } = generateFakes();
      ...
      describe('else', () => {
        it('no last speak', async () => {
          const { services, runtime, req, metaObj, topStorage } = generateFakes();
